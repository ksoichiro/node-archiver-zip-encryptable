...
ZipEncryptableArchiveOutputStream.prototype._smartStream = function(ae, callback) {
  ...
  function handleStuff() {
    ...
    // TODO: What's this padding?
    var f = Buffer.alloc(4);
    f.writeUInt32BE(0x0000ffff, 0);
    var ef = zipCrypto.encrypt(f);
    this.write(ef);

    var digest = process.digest().readUInt32BE(0);
    ae.setCrc(digest);
    ae.setSize(process.size());
    ae.setCompressedSize(process.size(true) + encryptionHeader.length + f.length);
